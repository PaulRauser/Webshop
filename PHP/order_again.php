...
function orderAgain($order_id)
{
    ...
    $sqlGetOrder = "SELECT id, fk_user, shipping_method, total_price, zip, address, country, city FROM orders WHERE id=?";
    ...
    $sqlAddNewOrder = "INSERT into orders (fk_user, shipping_method, total_price, zip, address, country, city) VALUES (?,?,?,?,?,?,?)";
    $peter = $conn->prepare($sqlAddNewOrder);
    $peter->execute([$fk_user, $shipping_method, $total_price, $zip, $address, $country, $city]);
